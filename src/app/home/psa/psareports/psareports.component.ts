...
export class PsareportsComponent implements OnInit  {
  ...
  constructor(private service: PsaService, private excel: ExcelService, private hrms: HrmsService, private route:Router ) { }
  ...
  download_pdf() {   
    ...
    for (let i in this.table_data) {
      ...
      for (let j in this.table_heading) {
        obj.push(this.table_data[i][this.table_heading[j]])
      }
      one.push(obj)
    }
    ...
    doc.autoTable(this.table_heading, one);
